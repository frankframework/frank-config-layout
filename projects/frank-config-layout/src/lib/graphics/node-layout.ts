...
export class NodeLayoutBuilder {
  ...
  run(): NodeLayout {
    this.layers = getRange(0, this.lb.numLayers).map((layerNumber) => this.createLayer(layerNumber));
    ...
    for (const p of positions) {
      positionMap.set(p.node.id, p);
    }
    return {
      width,
      height: this.dimensions.layerDistance * this.lb.numLayers,
      positions,
      positionMap,
  ...
  private setXCoordinates(): number {
    this.setInitialX();
    const allHorizontalIntervals: Interval[] = this.layers
      .flatMap((layer) => layer.positions)
      .map((p) => {
    ...
    for (const p of this.layers.flatMap((layer) => layer.positions)) {
      p.x = p.x! - minX;
    }
    return maxX - minX + 1;
  }

  private setInitialX(): void {
  ...
  initializeXFrom(subjectLayer: Layer, sourceLayer: Layer | null): void {
    if (sourceLayer !== null) {
      for (const p of subjectLayer.positions) {
    ...
    for (const [i, p] of subjectLayer.positions.entries()) {
      p.x = xCoords[i];
    }
  }
  ...
  widthOf(n: NodeForLayers): number {
    if (n.isIntermediate) {
