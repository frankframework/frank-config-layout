...
export class Edge2LineCalculation {
  ...
  constructor(nodeLayout: NodeLayout, d: NodeAndEdgeDimensions) {
    this.createNodes(nodeLayout, d);
    for (const edge of nodeLayout.edges) {
    ...
    for (const edge of this.edges) {
      this.edge2lineMap.set(
        getKey(edge),
        new Line(this.nodeMap.get(edge.from.id)!.getPointFor(edge), this.nodeMap.get(edge.to.id)!.getPointFor(edge)),
      );
    }
  }

  private createNodes(nodeLayout: NodeLayout, d: NodeAndEdgeDimensions): void {
    for (const p of nodeLayout.positions) {
  ...
  private connectEdge(edge: EdgeForLayers): void {
  ...
  private initNode(n: ConnectedPlacedNode, d: NodeAndEdgeDimensions): void {
    n.sortConnectedEdges(d);
  }

  getOriginalEdges(): EdgeForLayers[] {
    return this.edges;
  }
...
class ConnectedPlacedNode {
  ...
  constructor(readonly node: PlacedNode) {}

  connectEdgeToTop(edge: EdgeForLayers, xOtherSide: number): void {
  ...
  private getDirection(edge: EdgeForLayers): RelativeDirection {
  ...
  connectEdgeToBottom(edge: EdgeForLayers, xOtherSide: number): void {
    const connector: Connector = new Connector(this.getDirection(edge), xOtherSide);
    this.edge2connectorBottom.set(getKey(edge), connector);
    this.connectorsBottom.push(connector);
  }

  sortConnectedEdges(d: NodeAndEdgeDimensions): void {
    ...
    for (const [index, coord] of xtop.entries()) {
...
export function bottomComparator(first: Connector, second: Connector): number {
  const result = first.otherSideX - second.otherSideX;
  if (result === 0) {
