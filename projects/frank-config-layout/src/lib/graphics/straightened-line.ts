...
export function straighten(
  ...
  lineObstacleChecker: (idForObstacles: string, line: Line) => boolean,
  boundsChecker: (id: string, line: Line) => boolean,
): StraightenedLine[] {
  const result: StraightenedLine[] = [];
  let index = 0;
  while (index < segments.length) {
    ...
    if (next.isLayerPassage()) {
      ...
      if (replacements.every((replacement) => lineObstacleChecker(next.idStart, replacement.line))) {
        result.pop();
        for (const replacement of replacements) {
          console.log(`Replacement: ${replacement.idStart}, ${replacement.idEnd}`);
          joinAdd(result, replacement, boundsChecker);
        }
        index += 2;
        handled = true;
      }
    ...
    if (!handled) {
      joinAdd(result, next, boundsChecker);
      ++index;
    }
  }
  return result;
...
function joinAdd(
  ...
  console.log(`Existing segment start: ${existingSegments.map((segment) => segment.idStart)}`);
  console.log(`Existing segment end: ${existingSegments.map((segment) => segment.idEnd)}`);
  if (existingSegments.length === 0) {
    existingSegments.push(newSegment);
  } else {
    const joined: StraightenedLine = existingSegments.at(-1)!.toJoinedWith(newSegment);
    if (joined.replacedNodes.every((id) => boundsChecker(id, joined.line))) {
