...
export function createLayoutLineSegmentFromEdge(
  fromNode: PlacedNode,
  toNode: PlacedNode,
  edge: EdgeForLayers,
  line: Line,
...
export class Layout {
  ...
  constructor(
    ...
    readonly derivedEdgeLabelDimensions: DerivedEdgeLabelDimensions,
  ) {
    ...
    this.layoutLineSegments = calc
      .getOriginalEdges()
      .map((e) =>
        createLayoutLineSegmentFromEdge(
  ...
  private addVerticalLineSegmentsForIntermediateNodes(nodes: PlacedNode[]): void {
    ...
    for (const indexFirst in lineSegments) {
      for (const indexSecond in lineSegments) {
        if (indexSecond > indexFirst) {
          ...
          if (layersCoincide && this.edgesCross(lineSegments[indexFirst], lineSegments[indexSecond])) {
            ++result;
          }
        }
      }
...
function compareByX(first: LayoutLineSegment, second: LayoutLineSegment): number {
  const result = first.line.startPoint.x - second.line.startPoint.x;
  if (result !== 0) {
    return result;
  }
