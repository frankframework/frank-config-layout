...
describe('Straighten', () => {
  it('When no obstacles are hit and when we stay within boundaries, then the segments are reduced to a single segment', () => {
    ...
    const result: StraightenedLine[] = straighten(segments, () => true);
    expect(result.length).toEqual(1);
  ...
  it('When straightened lines do not stay within bounds or when boundaries are hit, then the original segments are kept', () => {
    ...
    const result: StraightenedLine[] = straighten(segments, () => false);
    expect(result.length).toEqual(3);
    expect(result.map((segment) => segment.idStart)).toEqual(['Start', 'i1', 'i1']);
    expect(result.map((segment) => segment.idEnd)).toEqual(['i1', 'i1', 'End']);
    expect(
      result
  ...
  it('When obstacles are not hit but fully straightened line not within bounds, then only the layer passage is eliminated', () => {
    const segments: StraightenedLine[] = [
      StraightenedLine.create('Start', 'i1', new Line(new Point(0, 0), new Point(1, 9))),
      StraightenedLine.create('i1', 'i1', new Line(new Point(1, 9), new Point(1, 11))),
      StraightenedLine.create('i1', 'End', new Line(new Point(1, 11), new Point(0, 20))),
    ...
    const result: StraightenedLine[] = straighten(
      segments,
      (_, line) => line.startPoint.x != 0 || line.endPoint.x != 0,
    );
    expect(result.length).toEqual(2);
    expect(result.map((segment) => segment.idStart)).toEqual(['Start', 'i1']);
    expect(result.map((segment) => segment.idEnd)).toEqual(['i1', 'End']);
    expect(
      result
        .map((segment) => segment.line)
        .map((line) => [line.startPoint.x, line.startPoint.y, line.endPoint.x, line.endPoint.y]),
