...
function addNode(id: string, text: string, g: OriginalGraph): void {
  g.addNode({
    id,
    text,
    // Dummy value
...
describe('Layout', () => {
  it('Test with intermediate nodes', () => {
    ...
    expect(layout.getLayoutLineSegments().map((lls) => lls.line.endPoint.x)).toEqual([60, 150, 75, 165, 120, 120, 75]);
