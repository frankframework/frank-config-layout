...
function newNode(id: string, g: Graph<WithId, Connection<WithId>>): void {
  g.addNode({ id });
}

function newEdge(fromId: string, toId: string, g: Graph<WithId, Connection<WithId>>): void {
...
function checkNodePointsTo(fromId: string, toIds: string[], g: Graph<WithId, TestEdge>): void {
...
function checkNodeReachedFrom(toId: string, fromIds: string[], g: Graph<WithId, TestEdge>): void {
  const to: WithId = g.getNodeById(toId);
  const predecessors: readonly WithId[] = g.getPredecessors(to);
